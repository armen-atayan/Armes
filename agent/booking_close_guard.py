...
_BOOKED = re.compile(
    r'\b(?:подтверждаю|подтверждаем|подтвержден[аоы]?|'
    r'записал[аи]?|записываю|записаны|записан|записана|запись\s+(?:готова|оформлена|создана)|'
    r'забронировал[аи]?|забронирован[аоы]?|бронируем|оформил[аи]?|оформлен[аоы]?|готово)\b',
    re.I,
)
...
def booking_close_reason(text: str, outcome: str) -> str | None:
    ...
    if outcome == 'agreed' and (not text or _NEGATIVE.search(text) or
        ...
        return ('Не завершай звонок: в последней реплике нет явного согласия организации на запись или бронь. '
                'Не считай неразборчивый текст согласием. Ответь на открытый вопрос; '
                'при необходимости попроси повторить. Согласие Армена в Telegram не заменяет ответ организации.')
    return None
