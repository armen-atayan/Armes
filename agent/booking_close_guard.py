...
_BOOKED = re.compile(r'\b(?:подтверждаю|подтверждаем|подтвержден[аоы]?|записал[аи]?|записаны|записан|записана|забронировал[аи]?|забронирован[аоы]?|бронируем|оформил[аи]?)\b', re.I)
_NATURAL_YES = re.compile(
    r'^(?:в\s+принципе\s+)?да(?:[\s,]+(?:могу|можем|все\s+нормально|нормально|спасибо))*[\s.!…]*$',
    re.I,
)
