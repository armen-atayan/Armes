...
try:
    ...
    logger.warning("soundfile package not found — Krisp debug audio logging disabled")
...
class KrispVivaFilterFrameProcessor(rtc.FrameProcessor[rtc.AudioFrame]):
    ...
    def __init__(
        ...
        Args:
            ...
            debug_audio_dir: Directory to save ``do_<ts>.ogg`` / ``posle_<ts>.ogg`` debug
                recordings for each session. When ``None`` (default), no files are written.
        ...
        # Debug audio recording: only active when debug_audio_dir is provided
        self._debug_dir: Path | None = Path(debug_audio_dir) if debug_audio_dir is not None else None
        self._debug_lock = threading.Lock()
        self._debug_before: list[np.ndarray] = []
        self._debug_after: list[np.ndarray] = []
        self._debug_session_ts = datetime.now().strftime("%Y%m%d_%H%M%S")

        # Suppress repeated "session unavailable" log spam
    ...
    def _process(self, frame: rtc.AudioFrame) -> rtc.AudioFrame:
        ...
        if self._session is None or self._sample_rate != frame.sample_rate:
            try:
                ...
                if not self._session_error_logged:
                    logger.error(
                        f"Cannot create Krisp session for {frame.sample_rate}Hz, "
                        f"passing through all frames: {e}"
                    )
        ...
        try:
            ...
            # Accumulate debug buffers (do_ / posle_)
            if self._debug_dir is not None and _SOUNDFILE_AVAILABLE:
                with self._debug_lock:
                    self._debug_before.append(audio_samples.copy())
                    self._debug_after.append(np.array(filtered_samples, dtype=np.int16).copy())
    ...
    def _flush_debug_audio(self) -> None:
        """Save accumulated before/after audio buffers as OGG files to the logs directory.

        Files are named ``do_<timestamp>.ogg`` and ``posle_<timestamp>.ogg``.
        No-op if soundfile is unavailable or no audio was captured.
        """
        if self._debug_dir is None or not _SOUNDFILE_AVAILABLE:
            return

        with self._debug_lock:
            before_chunks = self._debug_before[:]
            after_chunks = self._debug_after[:]
            self._debug_before.clear()
            self._debug_after.clear()
        ...
        try:
            ...
            ts = self._debug_session_ts

            before_pcm = np.concatenate(before_chunks).astype(np.float32) / 32768.0
            after_pcm = np.concatenate(after_chunks).astype(np.float32) / 32768.0

            before_path = self._debug_dir / f"do_{ts}.ogg"
            after_path = self._debug_dir / f"posle_{ts}.ogg"

            _sf.write(str(before_path), before_pcm, sample_rate, format="OGG", subtype="VORBIS")
            _sf.write(str(after_path), after_pcm, sample_rate, format="OGG", subtype="VORBIS")

            logger.info(f"Krisp debug audio saved: {before_path.name}, {after_path.name}")
        except Exception:
            logger.exception("Failed to save Krisp debug audio files")
